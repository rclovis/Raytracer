...
mat::Matrix<float, 1, 3> Shading::shadingPipelineRay (std::vector<IPrimitives*> primitives, std::vector<ILights*> lights, normalRay ray, cameraRay camera)
{
    ...
    for (auto &light : lights) {
        ...
        mat::Matrix<float, 1, 3> tmp = mixColor(objectColor, lightColor.color, lightColor.intensity) * lightColor.intensity * primitives[ray.primitiveId]->getMaterial().transparency;
        ...
        finalColor += tmp;
        // finalColor += primitives[ray.primitiveId]->getMaterial().color *  (1 - primitives[ray.primitiveId]->getMaterial().transparency);
    }
...
mat::Matrix<float, 1, 3> Shading::shadingPipeline (std::vector<IPrimitives*> primitives, std::vector<ILights*> lights, std::vector<normalRay> ray, cameraRay camera)
{
    ...
    for (auto &r : ray) {
        if (t == 0) {
            finalColor = shadingPipelineRay(primitives, lights, r, camera);
        } else {
            finalColor = mixColor(finalColor, shadingPipelineRay(primitives, lights, r, camera), 1 - opacity);
        }
        opacity += primitives[r.primitiveId]->getMaterial().transparency * (1 - opacity);
        if (opacity >= 1) break;
        t++;
    }
